...
public class CurlMesh {
    ...
    // curl position and one for the direction from the position
    // given. Comes handy once playing around with different
    ...
    // Bounding rectangle for this mesh. mRectangle[0] = top - left corner,
     ...
     *
     * @param maxCurlSplits
     *          Maximum number curl can be divided into. The bigger the value the smoother curl will
     *          be. With the cose of having more polygons for drawing.
     *
     * */
    public CurlMesh(int maxCurlSplits) {
        // There really is no use for 0 splits.
        mMaxCurlSplits = maxCurlSplits < 0 ? 1 : maxCurlSplits;
        ...
        if (DRAW_SHADOW) {
            final int count = 2 * (mMaxCurlSplits + 2);
            mArrSelfShadowVertices = new Array<>(count);
            mArrDropShadowVertices = new Array<>(count);
            mArrTempShadowVertices = new Array<>(count);
        ...
        // to two corners and curl consists of max mMaxCurlSplits lines each outputting 2 vertices.
